...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Split manifest whose development/test assignment is carried over for "
            "every retained record. Use this whenever an earlier partition was "
            "already used for model or threshold selection: recomputing the "
            "grouped split after records are excluded reorders the groups and can "
            "move records from development into test, which would retroactively "
            "invalidate every development-only selection decision."
        ),
    )
    ...
    if args.inherit_splits:
        inherited_path = args.inherit_splits.resolve()
        with inherited_path.open(encoding="utf-8-sig", newline="") as stream:
            inherited = {
                row["qa_id"]: row["split"] for row in csv.DictReader(stream)
            }
        missing = sorted(
            record["qa_id"] for record in retained if record["qa_id"] not in inherited
        )
        if missing:
            raise SystemExit(
                f"--inherit-splits manifest {inherited_path} has no assignment for "
                f"{missing}; every retained record must already carry one"
            )
        unknown = sorted(
            {inherited[record["qa_id"]] for record in retained}
            - {"development", "test"}
        )
        if unknown:
            raise SystemExit(f"--inherit-splits manifest has unknown splits {unknown}")
        assignments = {
            record["qa_id"]: inherited[record["qa_id"]] for record in retained
        }
        split_provenance = {
            "method": "inherited",
            "source_manifest": str(inherited_path),
            "source_manifest_sha256": sha256(inherited_path),
        }
    else:
