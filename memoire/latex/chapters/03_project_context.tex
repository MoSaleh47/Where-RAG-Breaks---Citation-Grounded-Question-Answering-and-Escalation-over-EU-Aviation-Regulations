...
This project was carried out during an apprenticeship at the French
\textbf{Direction g\'{e}n\'{e}rale de l'Aviation civile} (DGAC), within the
\textbf{Direction de la s\'{e}curit\'{e} de l'Aviation civile} (DSAC), in the
\textbf{Mission \'{e}valuation et am\'{e}lioration de la s\'{e}curit\'{e}}
(MEAS) and specifically its \textbf{\'{E}valuation des risques et analyses de
s\'{e}curit\'{e}} (ERAS) division.

The placement matters to this work for one reason: ERAS is, in the French
context, an office that operates the regime this system answers questions about.
Its remit covers the identification of hazards to civil aviation, the collection
and analysis of occurrence reports, contribution to the State Safety Programme,
and liaison with the safety planning of the \ac{EASA}. Regulation~(EU)~No~376/2014
is not an abstract object of study there; it is the instrument under which the
work of the division is conducted. The practitioners whose reading of citation
labels is reported in Chapter~\ref{chap:results} are people who work with this
regime professionally, and the observation that some of those labels could not be
resolved carries more weight for having come from them.

\paragraph{What this project is, and what it is not.}
Two statements are necessary for the reader to place this work correctly.

First, \textbf{the assigned apprenticeship mission was distinct from the subject
of this m\'{e}moire.} The day-to-day assignment was software development in
Python on other matters. This project is a Final Year Project undertaken in the
context of that placement and informed by it; it was not a deliverable requested
by the division, and nothing in it should be read as a system commissioned,
adopted, or deployed by the DGAC.

Second, \textbf{no internal DGAC material was used at any stage.} The entire
corpus is the publicly available \ac{EASA} Easy Access Rules consolidation of
Regulation~(EU)~No~376/2014~\cite{easa2022easyaccess376}; the benchmark questions
are synthetic; no occurrence report, no database extract, and no internal
document of the organisation enters the pipeline, the evaluation, or this
document. The confidentiality question that normally attends an
industry-hosted project therefore does not arise on the data side, and the
transmission of text to a hosted model, discussed in
Section~\ref{sec:method-ethics}, concerns published regulatory material only.

\draftnote{The industry supervisor is not named: the supervisor who began the
apprenticeship has left the division and a successor is expected in early
September 2026. Add the successor's role, and the name if they consent, before
submission. Confirm with the division that naming the DGAC, the DSAC, MEAS and
ERAS in this document is acceptable to them, even though no internal material is
used.}
...
Understanding why this domain is difficult for an automated system requires understanding how the material is structured. The consolidated publication is not one document. As parsed and validated in Chapter~\ref{chap:method}, it comprises the four distinct legal instruments and the body of guidance set out in Table~\ref{tab:context-sources}:
...
Table~\ref{tab:context-risks} lists the principal risks identified at the outset of the project and the mitigation applied to each.
