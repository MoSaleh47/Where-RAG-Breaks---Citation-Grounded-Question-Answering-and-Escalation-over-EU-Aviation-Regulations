...
The seventh of these deserves emphasis, because it is the finding that reframed the project. Three questions in the evaluation set resisted retrieval even at a candidate depth of twenty, and it would have been natural to report that as a system ceiling. Examined individually they were three different problems, and the independent review later settled which was which: one question was defective and no reviewer accepted it, while the panel accepted the other two, making those genuine retrieval failures rather than label defects. An earlier internal reading had attributed one of them to incomplete gold evidence; the reviewers disagreed, and their verdict stands. Reporting all three as a single ceiling would have been a measurement error rather than a result, and that observation is what motivated the validity protocol that now governs the whole evaluation.
...
Chapter~\ref{chap:results} reports the frozen benchmark and the review that produced it, retrieval and reranking on reviewed labels, the historical baselines re-scored under a corrected citation evaluator, evidence-constrained generation with its verification, and the calibration of the escalation policy.
